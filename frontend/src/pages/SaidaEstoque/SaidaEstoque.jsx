...
import { saidaAPI } from "../../service/api";
import "./SaidaEstoque.css";

const MOTIVOS = {
  Saída: ["Uso interno", "Ajuste manual"],
  Venda: ["Venda"],
  Perda: ["Produto danificado", "Perda"],
};
...
function SaidaEstoque() {
  ...
  async function carregarProdutos() {
    try {
      const lista = await saidaAPI.listarProdutos();
      setProdutos(lista);
    } catch {
  ...
  async function carregarHistorico() {
    try {
      const lista = await saidaAPI.listar();
      setHistorico(lista.slice(0, 10)); // últimas 10
    } catch {
      // silencioso
    }
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const resultado = await saidaAPI.registrar(payload);
      const nomeProduto = resultado.produto_nome || `Produto #${payload.produto_id}`;
    ...
    <section>
      <div className="page-card">
        ...
        <div className="movement-buttons saida-buttons">
          {Object.keys(MOTIVOS).map((tipo) => (
            <button
              key={tipo}
              className={`movement-type ${tipoAtivo === tipo ? "active" : ""}`}
              type="button"
      ...
      {historico.length > 0 && (
        <div className="page-card" style={{ marginTop: "1.5rem" }}>
          <h3 style={{ marginBottom: "1rem", color: "var(--text-primary, #1e293b)" }}>
            Últimas saídas
          </h3>
          <table className="saida-table">
            <thead>
              <tr>
                <th>Produto</th>
                <th>Qtd</th>
